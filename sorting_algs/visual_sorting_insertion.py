...
while running == 1:
    ...
    for x in list1:
        ...
        currentnum = x
        if numsbefore != 0:
            for x in list1[:numsbefore]: #for everything up to x
                if currentnum < x:
                    num1 = num1 + 1
                if num1 == len(list1[:numsbefore]): #if currentnum is smaller than ALL of numbers before, put it at index 0
                    list1.remove(currentnum)
                    list1.insert(0, currentnum)
                    num1 = 0
                    #continue #skip next section
                if currentnum < x and list1.index(currentnum) > list1.index(x):
                    list1.remove(currentnum)
                    list1.insert(list1.index(x), currentnum)
            num1 = 0 #I forgot to reset it
